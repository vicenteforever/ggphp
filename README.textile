...
目标：提供框架结构和思路，除了一些基本的规则，具体项目不耦合与核心框架，没有过多的框架限制，可以根据你自己的需求自由扩展
实现：框架结构 url路由 AOP面向切面编程框架(日志、运行报告、权限认证) 分布式应用(memcache) nosql存储 单元测试
集成：ORM(phpDataMapper) 图像处理(easyphpthumbnail) 模板(smarty) 格式化文本(textile, markdown)
...
# language 翻译文件：GG_DIR/language/{语言代码}.php 和 APP_DIR/language/{语言代码}.php
